...
//checking minimum of the two values
bool minimum(double divergence, double jeff) {
	if (divergence < jeff) return 0;
	else return 1;
}


//calculating jeffrey divergence corresponding to particular class
double jeffrey(int i) {
	double sum = 0;
	double avg = 0;
	for (int j = 0; j < 4; j++) {
		avg = (freecode[j] + encodings[i][j]) / 2;
		sum += (freecode[j] * log10(freecode[j]/avg) + encodings[i][j] * log10(encodings[i][j]/avg));
	}
...
int main() {
	...
	//five classes of x-ray 
	string className[5] = { "skull","foot","hand","knee","hip" };


	//creating class folders for saving classified images
	for (int i = 0; i < 5; i++) {

		string folderName = className[i];
	...
	while(n){
		...
		//reading input image
		string fileName = inputfiles[25 - n];
		...
		int classCode;
		double divergence = std::numeric_limits<double>::max();
		int mini = 0;

		//checking jeffrey divergence of the image with each class and finding the minimum
		for (int i = 0; i < 5; i++) {
			if (minimum(divergence, jeffrey(i))) {
				mini = i;
				divergence = jeffrey(i);
			}
		}

		//displaying classification result and storing into class folders
		cout << fileName << " " << "class =" << className[mini] << endl;
		cout << " " << endl;

		string folderName = className[mini];
